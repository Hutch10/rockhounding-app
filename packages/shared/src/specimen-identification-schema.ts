 ...
 */
export function shouldAutoAccept(confidence: number, imageQuality: ImageQuality): boolean {
...
export function requiresManualIdentification(
  ...
  if (alternatives.length > 0) {
    ...
    if (topAlt !== undefined && confidence - topAlt.confidence < 0.1) {
      return true;
    }
  }
 ...
 */
export function calculateImageQualityScore(metrics: ImageQualityMetrics): number {
...
export function assessImageQuality(score: number): ImageQuality {
  if (score >= 0.9) return ImageQuality.EXCELLENT;
  if (score >= 0.75) return ImageQuality.GOOD;
  if (score >= 0.6) return ImageQuality.ACCEPTABLE;
  if (score >= 0.4) return ImageQuality.POOR;
...
export function calculatePipelineMetrics(session: IdentificationCaptureSession): {
  ...
  return {
    ...
    classification_to_validation_ms:
      session.validation_timestamp != null && session.completed_at != null
...
export function validateCaptureForSync(session: IdentificationCaptureSession): {
  ...
  if (
    ...
    (session.final_material_id === '' || session.final_material_id == null)
  ) {
...
export function buildFindLogFromCapture(input: CreateFindLogFromCapture): {
  ...
  const materialName =
    validationEvent?.correct_material_id != null && validationEvent.correct_material_id !== ''
  ...
  const qualityRating = Math.min(
    5,
    Math.max(1, Math.ceil(classificationResult.confidence_score * 5))
  );
  ...
  const notes = [
    ...
    validationEvent?.action === ValidationAction.ACCEPT ? 'User validated identification' : '',
